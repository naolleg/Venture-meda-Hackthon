...
# Step 3: Group the data by city, product, and date (sum the quantity sold)
grouped_data = df.groupby(['City', 'Product', 'SalesDate']).agg({'Quantity': 'sum'}).reset_index()

# Step 4: Rename columns to match Prophet's format
grouped_data.rename(columns={'SalesDate': 'ds', 'Quantity': 'y'}, inplace=True)
